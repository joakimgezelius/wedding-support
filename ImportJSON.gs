...
/**
 * Below follows a class adaptation of the library:
 **/
...
class JSONImporter {
   ...
   */
  static parseJSONObject(object, query, options, includeFunc, transformFunc) {
    ...
    JSONImporter.parseData(headers, data, "", 1, object, query, options, includeFunc);
    JSONImporter.parseHeaders(headers, data);
    JSONImporter.transformData(data, options, transformFunc);
    
    return JSONImporter.hasOption(options, "noHeaders") ? (data.length > 1 ? data.slice(1) : new Array()) : data;
  }
   ...
   */
  static parseData(headers, data, path, rowIndex, value, query, options, includeFunc) {
    var dataInserted = false;
    
    if (TypeUtils.isObject(value)) {
      for (key in value) {
        if (JSONImporter.parseData(headers, data, path + "/" + key, rowIndex, value[key], query, options, includeFunc)) {
          dataInserted = true; 
        }
      }
    } else if (Array.isArray(value) && TypeUtils.isObjectArray(value)) {
      for (var i = 0; i < value.length; i++) {
        if (JSONImporter.parseData(headers, data, path, rowIndex, value[i], query, options, includeFunc)) {
          dataInserted = true;
          rowIndex++;
        }
      }
   ...
   */
  static parseHeaders(headers, data) {
    data[0] = new Array();
   ...
   */
  static transformData(data, options, transformFunc) {
   ...
   */
  static includeXPath(query, path, options) {
    if (!query) {
      return true; 
    } else if (Array.isArray(query)) {
      for (var i = 0; i < query.length; i++) {
        if (JSONImporter.applyXPathRule(query[i], path, options)) {
          return true; 
        }
      }  
    } else {
      return JSONImporter.applyXPathRule(query, path, options);
    }
   ...
   */
  static applyXPathRule(rule, path, options) {
   ...
   *    debugLocation: Prepend each value with the row & column it belongs in
   */
  static defaultTransform(data, row, column, options) {
    if (!data[row][column]) {
      if (row < 2 || JSONImporter.hasOption(options, "noInherit")) {
        data[row][column] = "";
      } else {
    ...
    if (!JSONImporter.hasOption(options, "rawHeaders") && row == 0) {
      if (column == 0 && data[row].length > 1) {
        JSONImporter.removeCommonPrefixes(data, row);  
      }
      
      data[row][column] = StringUtils.toTitleCase(data[row][column].toString().replace(/[\/\_]/g, " "));
    }
    
    if (!JSONImporter.hasOption(options, "noTruncate") && data[row][column]) {
      data[row][column] = data[row][column].toString().substr(0, 256);
    }

    if (JSONImporter.hasOption(options, "debugLocation")) {
   ...
   */
  static removeCommonPrefixes(data, row) {
    var matchIndex = data[row][0].length;

    for (var i = 1; i < data[row].length; i++) {
      matchIndex = StringUtils.findEqualityEndpoint(data[row][i-1], data[row][i], matchIndex);
