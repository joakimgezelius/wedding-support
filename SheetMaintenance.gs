...
const orgChargesSheetName = "Org Charges";
...
const emailsSheetName = "Emails to Clients";
...
function onInstallOrgChargesSheet() {
  SheetMaintenance.installSheetTemplate(templateSheetUrl, orgChargesSheetName);
}
...
function onInstallInstructionSheets() {
  SheetMaintenance.installSheetTemplate(templateSheetUrl, instructionsSheetName, 1);
  SheetMaintenance.installSheetTemplate(templateSheetUrl, orgChargesSheetName, 2);
  SheetMaintenance.installSheetTemplate(templateSheetUrl, emailsSheetName, 3);
}
...
class SheetMaintenance {
  ...
  static installSheetTemplate(templateUrl, sheetName, newSheetTabOrder = 1) {
    ...
    activeSpreadSheet.nativeSpreadsheet.moveActiveSheet(newSheetTabOrder);
    trace(`< SheetMaintenance.installSheetTemplate ${sheetName}`);
  }
