...
class HubSpot {
  ...
  static getClientData() {
    trace("getClientData");
    let invoiceNo = Range.getByName("InvoiceNo","Client Data").values;
    trace(`Invoice Number : ${invoiceNo}`);
    ...
    trace(`HubSpot.getDeal --> ${response.getContentText()}`);
  ...
  static listDeal() {
    ...
    let result = data;
    let sheet = SpreadsheetApp.getActiveSheet();
    let header = ["Amount", "Close Date", "Create Date", "Deal Name", "Deal Description", "Deal Owner", "Deal Type", "Deal Stage", "Departure Date", "Forecast Amount", "Forecast Category", "Forecast Probabilty", "HubSpot Team", "Last Modified Date", "Next Step", "Number of Contacts", "Priority", "Pipeline"];  // 18 deal properties
    let items = [header];
