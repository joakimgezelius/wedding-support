...
class HubSpot {
  ...
  static listContact(contactId) {
    ...
    console.log(data);
    let result =data;
    let range = SpreadsheetApp.getActive().getRangeByName("ClientData");
  ...
  static getClientData() {
    ...
    let result = data;
    let range = SpreadsheetApp.getActive().getRangeByName("ClientData");
    let header = ["Invoice ID", "Estimate ID", "Amount", "Close Date", "Create Date", "Deal Name", "Deal Description", "Deal Owner", "Deal Type", "Deal Stage", "Departure Date", "Forecast Amount", "Forecast Category", "Forecast Probabilty", "HubSpot Team", "Last Modified Date", "Next Step", "Number of Contacts", "Priority", "Pipeline"];  // 20 deal properties
    let items = [header];
    let props = ["invoice_id","estimate_id","amount","closedate","createdate","dealname","description","hubspot_owner_id","dealstage","dealtype","departure_date","hs_forecast_amount","hs_manual_forecast_category","hs_forecast_probability","hubspot_team_id","hs_lastmodifieddate","hs_next_step","num_associated_contacts","hs_priority","pipeline" ];
