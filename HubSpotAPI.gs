...
class HubSpot {
  ...
  static listContact(contactId) {
    ...
    sheet.getRange(26,3,items[0].length,items.length).setValues(Object.keys(items[0]).map ( function (columnNumber) {
      return items.map( function (row) {
        sheet.getRange(26,2,items[0].length,items.length).setValue("Contact");
  ...
  static getClientData() {
    ...
    let result = data;
    let sheet = SpreadsheetApp.getActiveSheet();
    let header = ["Invoice ID", "Estimate ID", "Amount", "Close Date", "Create Date", "Deal Name", "Deal Description", "Deal Owner", "Deal Type", "Deal Stage", "Departure Date", "Forecast Amount", "Forecast Category", "Forecast Probabilty", "HubSpot Team", "Last Modified Date", "Next Step", "Number of Contacts", "Priority", "Pipeline"];  // 19 deal properties
    let items = [header];
    ...
    sheet.getRange(6,3,items[0].length,items.length).setValues(Object.keys(items[0]).map( function(columnNumber) {
      return items.map( function(row) {
        sheet.getRange(6,2,items[0].length,items.length).setValue("Deal");
        return row[columnNumber];
      });
    }));
