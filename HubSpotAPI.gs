...
class HubSpot {
  ...
  static listContact(contactId) {
    ...
    sheet.getRange(8,19,items.length,items[0].length).setValues(items);
  }
  ...
  static dealToContact(dealId) {
    ...
    for(i; i < rl; ++i) {
      if( result[i].type == "deal_to_contact")
      {
        ...
        trace(`Found Contact ID : ${contactId}`);
  ...
  static getClientData() {
    ...
    sheet.getRange(8,1,items.length,items[0].length).setValues(items);
